...
- 用户端与管理端新增圆角浏览器标签页图标。
- 新增 `32x32`、`64x64` favicon 与 `180x180` Apple Touch Icon，统一放在各自项目的 `src/static` 目录。
- 标签页图标引用增加缓存版本号 `v=20260515r2`，便于浏览器刷新图标缓存。

### 文档与安全

- README 补充浏览器标签页图标文件位置与引用方式。
- `.gitignore` 忽略仓库根目录导出的本地源图和临时 PNG，避免误提交高清源图或临时图片。

## 2026-04-27

### 部署文档

- README 增加管理端登录“密钥无效”排障：说明 `403` 才是密钥不匹配，`500` 可能是后端运行错误。
- 宝塔部署文档补充 SQLite 外置数据库目录权限要求：`data/` 目录和 `timeline.db` 必须对 PHP-FPM 运行用户可写，WAL 模式会在同目录创建 `-wal` / `-shm` 文件。
- 补充权限修复示例和 `/verify-admin-key` 接口验证方式，避免把数据库目录不可写误判为管理员密钥错误。
